...
just groups the records into, for example, <i>good/bad</i>. Principal components 
...
are assigned to the groups in such a way that the points are as similar to each other as
...
<center>$$withinness(C_j) = \sum_{\substack{j\in 1:ncol \\ i\in 1:nrow}}{(x_{ij} - \mu_j)^2} $$</center>
...
is to come up with centroids such that, after cluster assignment, the residual 
sum squares are minimized. Following what we saw earlier, this can be defined
mathematically as

<br><center>$$minimize\ ||CW - X||^2 $$</center><br>

* C denotes a matrix of mean values of all columns in different clusters of dimension
`J` by `K` where J is the number of columns and K is the number of clusters.  
...
target="_blank">Machine Learning Redifined</a></u> expresses the concept in a much
better way.
...
The only constraint is that each of the data points 
...
```{r fig.height=9, fig.width=6, warning=FALSE, message=FALSE, eval = FALSE}
# ```{R}
